...
public class RBRegular {
	...
	public static void placeOrder(String price,String size,int buyOrSell,int moreOrLess) {
		...
		BaseUtils.id(ProLoading.rbRegular).click();
...
//				BaseUtils.sleep(20);
//				System.out.println(BaseUtils.toastBoolean("杠杆调整成功"));
				BaseUtils.id(ProLoading.price).clear();
				BaseUtils.id(ProLoading.price).sendKeys(price);
				BaseUtils.id(ProLoading.size).clear();
				BaseUtils.id(ProLoading.size).sendKeys(size);
...
//		System.out.println("count==="+count);
	...
	/**
	 * 
	 * @param xxxCurrency btc eos eth ltc xrp etc bnb ht1
	 */
	public static void exchangeCurrency(String xxxCurrency) {
		BaseUtils.id(ProLoading.contract).click();
		SwitchContract.switchContract();
		BaseUtils.id(ProLoading.rbRegular).click();
		...
		switch(xxxCurrency) {
			case "btc":
				BaseUtils.xpath(ProLoading.BTC).click();
				break;
			case "eos":
				BaseUtils.xpath(ProLoading.EOS).click();
				break;
			case "eth":
				BaseUtils.xpath(ProLoading.ETH).click();
				break;
			case "ltc":
				BaseUtils.xpath(ProLoading.LTC).click();
				break;
			case "xrp":
				BaseUtils.xpath(ProLoading.XRP).click();
				break;
			case "etc":
				BaseUtils.xpath(ProLoading.ETC).click();
				break;
			case "bnb":
				BaseUtils.xpath(ProLoading.BNB).click();
				break;
			case "ht1":
				BaseUtils.xpath(ProLoading.HT1).click();
				break;
			default:
				break;
		}
	...
	/**
	 * 
	 * @param money 交割合约资金分配
	 */
	public static void fundAllocations(String money) {
		BaseUtils.id(ProLoading.contract).click();
		SwitchContract.switchContract();
		BaseUtils.id(ProLoading.rbRegular).click();
		BaseUtils.xpath(ProLoading.r_hamburger).click();
		BaseUtils.xpath(ProLoading.fundAllocations).click();
		BaseUtils.id(ProLoading.fundAllocations_count).clear();
		BaseUtils.id(ProLoading.fundAllocations_count).sendKeys(money);
		BaseUtils.clickPress();
		System.out.println("往当前币种中，分配了"+money+"USDT");
	}
	/**
	 * 右上角的资金转入
	 * @param currency
	 */
	public static void transferOffunds(String currency) {
		BaseUtils.id(ProLoading.contract).click();
		SwitchContract.switchContract();
		BaseUtils.id(ProLoading.rbRegular).click();
		BaseUtils.xpath(ProLoading.r_hamburger).click();
		BaseUtils.xpath(ProLoading.transferOffunds).click();
		BaseUtils.sleep(5);
		...
		//点击系统的返回键
		BaseUtils.goBack();
		System.out.println("充值账户往交割合约中，转入了"+currency+"USDT");
	}
	/**
	 * 1.获取当前各个币种一共有多少仓位
	 * 2.获取委托中有多少委托的仓位
