...
public class BtcdClientImpl implements BtcdClient {
	...
	@Override
	public BigDecimal estimateFee() throws BitcoindException,
			CommunicationException {
		String estimatedFeeJson = rpcClient.execute(Commands.ESTIMATE_FEE.getName(), 2);
		BigDecimal estimatedFee = rpcClient.getParser().parseBigDecimal(estimatedFeeJson);
		return estimatedFee;
	}

	@Override
	public BigDecimal estimateSmartFee() throws BitcoindException, CommunicationException {
		String ret = this.rpcClient.execute("estimatesmartfee", 1);
		return JSON.parseObject(ret).getBigDecimal("feerate");
	}

	@Override
	public BigDecimal estimateFeeNoBlock() throws BitcoindException, CommunicationException {
		String ret = this.rpcClient.execute("estimatefee");
		return this.rpcClient.getParser().parseBigDecimal(ret);
	}
	...
	@Override
	public BlockData getBlockData(String headerHash) throws BitcoindException, CommunicationException {
		List params = CollectionUtils.asList(new Object[] {headerHash, 2});
		String blockJson = this.rpcClient.execute(Commands.GET_BLOCK.getName(), params);
		BlockData block = (BlockData) JSON.parseObject(blockJson, BlockData.class);
		return block;
	}
	...
	@Override
	public int generateToAddress(int count,String address) throws BitcoindException, CommunicationException {
		List<Object> params = CollectionUtils.asList(count,address);
		String ss = rpcClient.execute(Commands.GENERATE_TO_ADDRESS.getName(), params);
		return 1;
	}
	...
	public RawTransactionOverview getRawTransactionV(String txId) throws BitcoindException, CommunicationException {
		List params = CollectionUtils.asList(new Object[] {txId,1});
		String hexTransactionJson = this.rpcClient.execute(Commands.GET_RAW_TRANSACTION.getName(), params);
		return (RawTransactionOverview)JSON.parseObject(hexTransactionJson, RawTransactionOverview.class);
	}
	...
	@Override
	public TxOut getTxOut(String txid, int vout) throws CommunicationException, BitcoindException {
		List<Object> params = CollectionUtils.asList(new Object[]{txid, vout});
		String txDetail = this.rpcClient.execute(Commands.GET_TX_OUT.getName(), params);
		TxOut txout = (TxOut)this.rpcClient.getMapper().mapToEntity(txDetail, TxOut.class);
		return txout;
	}
	...
	@Override
	public SignatureResult signRawTransactionWithKey(String hexTransaction, List<Output> outputs, List<String> privateKeys)
			throws BitcoindException, CommunicationException {
		List params = CollectionUtils.asList(new Object[] {hexTransaction, privateKeys,outputs});
		String signatureResultJson = this.rpcClient.execute("signrawtransactionwithkey", params);
		SignatureResult signatureResult = (SignatureResult)JSON.parseObject(signatureResultJson,
				SignatureResult.class);
		return signatureResult;
	}
